...
- **Výstup**:
  ...
  - V logu (`core_logger/core_logger.log`): Záznamy o inicializaci loggeru, prohledávání sample, vyhledávání (např. "Loaded: m108-vel7-f44.wav (MIDI: 108, Vel: 7, Freq: 44100 Hz, Duration: 2.5s, Channels: 2 (stereo), Frames: 110250, Format: interleaved, needs float conversion)").
...
| `getDurationInSeconds(int index, Logger& logger) const` | `index` – index v seznamu, `logger` – reference na Logger | `double dur = io.getDurationInSeconds(idx, logger);` | Vrátí délku v sekundách (vypočteno jako frames/samplerate). Kontrola indexu jako u ostatních. | `double` | Délka v sekundách (např. `2.5`); chyba: exit(1) po logu |
...
| `getIsInterleavedFormat(int index, Logger& logger) const` | `index` – index v seznamu, `logger` – reference na Logger | `bool interleaved = io.getIsInterleavedFormat(idx, logger);` | Vrátí true pokud interleaved formát (standard pro WAV). Kontrola indexu jako u ostatních. | `bool` | `true` (interleaved), `false` (non-interleaved); chyba: exit(1) po logu |
| `getNeedsConversion(int index, Logger& logger) const` | `index` – index v seznamu, `logger` – reference na Logger | `bool needsConv = io.getNeedsConversion(idx, logger);` | Vrátí true pokud potřebuje konverzi do float (16-bit PCM → float). Kontrola indexu jako u ostatních. | `bool` | `true` (potřebuje konverzi), `false` (již float); chyba: exit(1) po logu |

**Příklad plného použití s gettery**:
```cpp
Logger logger("./");
SamplerIO io;  // Prázdný sampler
std::string dir = R"(c:\Users\jindr\AppData\Roaming\IthacaPlayer\instrument)";
io.scanSampleDirectory(dir, logger);  // Prohledá a zaloguje (s validací frekvence a detekce formátu)
...
if (idx != -1) {
    ...
    double duration = io.getDurationInSeconds(idx, logger);
    int channels = io.getChannelCount(idx, logger);
    bool isStereo = io.getIsStereo(idx, logger);
    
    // Metadata pro audio processing
    bool isInterleaved = io.getIsInterleavedFormat(idx, logger);
...
- **Komentář**: Spustí kompletní workflow (deleguje prohledávání do SamplerIO, vyhledá příklad MIDI 108/vel 7, použije gettery pro metadata včetně nových). Loguje všechny kroky včetně detailních informací o délce, kanálech, stereo módu, formátu a potřebě konverze. Vrátí 0 při úspěchu (chyby končí `std::exit(1)`). Použijte pro standalone test.
...
struct SampleInfo {
    ...
    bool interleaved_format;        // True pokud interleaved formát (standard pro WAV)
...
- Pro kompletní metadata z WAV souborů a analýzu formátu.
...
- **Příklad instancování a volání** (z `main.cpp`):
  ...
  logger.log("SamplerIO/scanSampleDirectory", "info", "Scanning sample directory: /path/to/dir");
  ```
...
- Chyby: Selhání inicializace (adresář, přístup, neplatný index v gettech, nekonzistentní frekvence, nepodporovaný formát) vede k logu a `std::exit(1)`.
- Thread-safety: Pouze logování je chráněno mutexem; sampler není navržen pro multi-threading.
- Rozšířená metadata: Program nyní prohledává a poskytuje přístup k délce, počtu kanálů, stereo detekci, počtu vzorků, formátu a potřebě konverze.
